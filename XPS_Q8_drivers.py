...
class XPS:
	...
	def __sendAndReceive (self, socketId, command):
		command = command.encode()  # fixed : add this line
		try:
			XPS.__sockets[socketId].send(command)
			ret = XPS.__sockets[socketId].recv(1024)
			while (ret.find(b',EndOfAPI') == -1):  # fixed ： turn 'EndofAPI' to b'EndofAPI'
				ret += XPS.__sockets[socketId].recv(1024)
		except socket.timeout:
			print('timeout')
			return [-2, 'timeout']
		except socket.error: # fixed :
			# print ('Socket error : ' + str(errString))
			print("socketerror")
			return [-2, 'socketerror']

		for i in range(len(ret)):
			if (ret[i] == 44):  #fixed : turn ret[i] == ',' to ret[i] == 44 (44 is the ascii no. of ',' )
